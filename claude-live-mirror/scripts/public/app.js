(function() {
  ...
  const SETTINGS_KEY = 'claude-live-mirror-settings';
  ...
  function initXterm() {
    ...
    // Mousedown on gutter: start drag (stops BOTH mouse + pointer propagation)
    ...
    // Drag: window capture phase + stopPropagation blocks xterm entirely
    ...
    function buildInlineWidget(c, stackIndex) {
      ...
      // Full comment body (expanded)
      const body = document.createElement('div');
      body.className = 'inline-comment-body';
      body.textContent = c.comment;
      widget.appendChild(body);

      // Action buttons (expanded, pending only)
      if (!c._submitted) {
    ...
    // Wheel scroll may not always fire xterm.onScroll synchronously
    ...
    // Hook gutter markers + inline comments + scroll button to terminal events
  ...
  function sendAll(withSubmit) {
    ...
    // Format comments + message as text
    const parts = [];
    ...
    // Inject into PTY via WebSocket (bracketed paste mode for literal newlines)
    const data = '\x1b[200~' + text + '\x1b[201~';
    terminalWs.send(JSON.stringify({ type: 'input', data }));

    // Send & Submit: press Enter after a short delay so paste is fully processed
    if (withSubmit) {
      setTimeout(() => {
        if (terminalWs && terminalWs.readyState === WebSocket.OPEN) {
    ...
    // Also notify server for poll-based consumers (e.g. /open-live-mirror command)
    fetch('/api/submit', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ comments: hasComments ? comments : [], message: message || undefined, batchId }),
    }).catch(() => { /* fetch may fail if server is shutting down */ });
  ...
  async function done() {
    try { await fetch('/api/done', { method: 'POST' }); } catch { /* server may already be closed */ }
    window.close();
  }
  ...
  document.addEventListener('mousedown', (e) => {
    ...
    // Collapse expanded inline comment on outside click
  ...
  function startDisconnectTimer() {
    ...
    disconnectTimer = setTimeout(() => {
      ...
      window.close();
      // Fallback: if window.close() is blocked (not opened by script), show notice
    ...
    commentWs.onmessage = (e) => {
      try {
        ...
        if (msg.type === 'comments' && msg.comments) {
          // Skip broadcasts from own submissions
